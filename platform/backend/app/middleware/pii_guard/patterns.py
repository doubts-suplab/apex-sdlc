"""PII regex pattern library — absorbed from ``governance/pii-guard/patterns.py``.

The platform middleware is **regex-only and offline**: it carries no AWS Comprehend layer, so these
patterns are the whole detector. Each entry is ``(label, compiled_regex)``; ``ALL_PATTERNS`` is
ordered most-specific-first so overlapping spans resolve to the tighter label.
"""
...
# All patterns — ordered by specificity (most specific first).
...
# Labels treated as high confidence — always redacted; short-match filtering does not apply.
